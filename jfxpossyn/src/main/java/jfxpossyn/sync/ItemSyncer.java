...
public class ItemSyncer {
	...
	public void syncItem(AppConfig config, SyncProgressListener listener) {
		...
		try {
			...
			if (prepareResponse.statusCode() != 201 && prepareResponse.statusCode() != 200) {
				throw new Exception("Gagal prepare sync. HTTP status: " + prepareResponse.statusCode() + ", response: "
			...
			if (prepareMap == null || !Boolean.TRUE.equals(prepareMap.get("success"))) {
				throw new Exception(
						"Prepare sync gagal: " + (prepareMap != null ? prepareMap.get("message") : "response kosong"));
			}
			...
			if (blockCount == 0) {
				...
				logger.info(String.format("donwloding %d blocks", blockCount));
				for (int block = 1; block <= blockCount; block++) {
					...
					if (itemsList != null && !itemsList.isEmpty()) {
						...
						// logger.info(String.format("Berhasil menyimpan %d item dari blok %d",
						// itemsToSave.size(), block));
					}
				}
			}
	...
	private void callFinishSync(HttpClient client, AppConfig config, int possyncId, boolean isCompleted,
		...
		if (response.statusCode() != 200) {
			throw new Exception(
					"Gagal finish sync. HTTP status: " + response.statusCode() + ", response: " + response.body());
		}
	...
	private List<Item> parseItems(List<Object> rawList) {
		...
		for (Object obj : rawList) {
			...
			if (itemIdVal != null) {
				item.setItemId(itemIdVal instanceof Number ? ((Number) itemIdVal).longValue()
						: Long.parseLong(itemIdVal.toString()));
			}

			Object codeVal = m.get("item_code");
			if (codeVal == null)
				codeVal = m.get("item_art");
			item.setItemArt(codeVal != null ? codeVal.toString() : "");

			Object nameVal = m.get("name");
			if (nameVal == null)
				nameVal = m.get("item_descr");
			...
			if (priceGrossVal == null)
				priceGrossVal = m.get("pricegross");
			if (priceGrossVal == null)
				priceGrossVal = priceVal;
			if (priceGrossVal != null) {
			...
			if (ctgIdVal != null) {
				item.setCtgId(ctgIdVal instanceof Number ? ((Number) ctgIdVal).intValue()
						: Integer.parseInt(ctgIdVal.toString()));
			}
			...
			if (unitIdVal != null) {
				item.setUnitId(unitIdVal instanceof Number ? ((Number) unitIdVal).intValue()
						: Integer.parseInt(unitIdVal.toString()));
			}
			...
			if (structIdVal != null) {
				item.setStructId(structIdVal instanceof Number ? ((Number) structIdVal).intValue()
						: Integer.parseInt(structIdVal.toString()));
			}
			...
			if (brandIdVal != null) {
				item.setBrandId(brandIdVal instanceof Number ? ((Number) brandIdVal).intValue()
			...
			Object barcodesVal = m.get("barcodes");
			if (barcodesVal instanceof List) {
				List<ItemBarcode> barcodeList = new ArrayList<>();
				for (Object bcObj : (List<?>) barcodesVal) {
					if (bcObj instanceof Map) {
						Map<String, Object> bcMap = (Map<String, Object>) bcObj;
						ItemBarcode ib = new ItemBarcode();

						Object bcIdVal = bcMap.get("itembarcode_id");
						if (bcIdVal != null) {
							ib.setItemBarcodeId(bcIdVal instanceof Number ? ((Number) bcIdVal).longValue()
									: Long.parseLong(bcIdVal.toString()));
						}

						Object bcItemIdVal = bcMap.get("item_id");
						if (bcItemIdVal != null) {
							ib.setItemId(bcItemIdVal instanceof Number ? ((Number) bcItemIdVal).longValue()
									: Long.parseLong(bcItemIdVal.toString()));
						} else {
							ib.setItemId(item.getItemId());
						}

						Object barcodeStrVal = bcMap.get("barcode");
						ib.setBarcode(barcodeStrVal != null ? barcodeStrVal.toString() : "");

						Object bcBrandIdVal = bcMap.get("brand_id");
						if (bcBrandIdVal != null) {
							ib.setBrandId(bcBrandIdVal instanceof Number ? ((Number) bcBrandIdVal).intValue()
									: Integer.parseInt(bcBrandIdVal.toString()));
						}
						...
						if (bcCreatedVal != null) {
							try {
								Instant instant = Instant.parse(bcCreatedVal.toString());
								ib.setCreatedAt(
										java.time.LocalDateTime.ofInstant(instant, java.time.ZoneId.systemDefault()));
							} catch (Exception e) {
								try {
									ib.setCreatedAt(java.time.LocalDateTime.parse(bcCreatedVal.toString()));
								} catch (Exception ex) {
								}
							}
						}

						Object bcDtTsVal = bcMap.get("datatimestamp");
						if (bcDtTsVal != null) {
							try {
								Instant instant = Instant.parse(bcDtTsVal.toString());
								ib.setDataTimestamp(
										java.time.LocalDateTime.ofInstant(instant, java.time.ZoneId.systemDefault()));
							} catch (Exception e) {
								try {
									ib.setDataTimestamp(java.time.LocalDateTime.parse(bcDtTsVal.toString()));
								} catch (Exception ex) {
								}
							}
						}
